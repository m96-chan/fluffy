/// Registers .vrm files as Gltf assets by delegating to Bevy's GltfLoader.
/// VRM 0.x and VRM 1.x are both valid GLB (binary GLTF 2.0) files.
...
impl Plugin for VrmLoaderPlugin {
    fn build(&self, app: &mut App) {
        app.preregister_asset_loader::<VrmGltfLoader>(&["vrm"]);
    }
    ...
    fn finish(&self, app: &mut App) {
        ...
        app.register_asset_loader(VrmGltfLoader(GltfLoader {
...
/// Thin wrapper around GltfLoader that handles .vrm extension.
#[derive(TypePath)]
pub struct VrmGltfLoader(GltfLoader);

impl AssetLoader for VrmGltfLoader {
